 ...
 *
 * Rewarded video is the only ad format -- no banners, no interstitials.
 * Everything sits behind a narrow interface so the underlying SDK can be
 * swapped without touching UI code.
 *
 * The App IDs come from `expo.extra.startIo` in app.json. Until they are set
 * `adsEnabled()` is `false`, every ad-gated control hides itself, and the
 * SDK is never initialised. Filling in `expo.extra.startIo` is the ONLY step
 * needed to go live -- no other code changes.
 */

import Constants from 'expo-constants';
import { Platform } from 'react-native';

import { pauseMusicForAd, resumeMusicAfterAd } from '@/game/audio/engine';
...
/** Resolved lazily: importing pulls in a Nitro module that Expo Go and web cannot provide. */
 ...
 *
 * Ad-gated UI must check this and hide itself entirely while it's `false`,
 * so the app never shows an ad affordance it can't back with a real ad.
 */
...
/**
 * Shows a rewarded video and resolves with whether the reward was actually
 * earned. Callers must only grant the reward when this resolves `true`.
 */
export function showRewarded(source: string): Promise<boolean> {
  reportEvent('rewarded_ad', { action: 'view', source });

  const module = loadModule();
  if (!module || !initialized) {
    // Without the native module there is no ad to watch. In development the
    // reward is granted anyway so the surrounding flows stay testable in
    // Expo Go; release builds always run through the real SDK.
    if (__DEV__) {
      console.warn(`[ads] no SDK for "${source}"; granting reward in dev only`);
      reportEvent('rewarded_ad', { action: 'reward', source });
      return Promise.resolve(true);
    }
    reportEvent('rewarded_ad', { action: 'error', source });
